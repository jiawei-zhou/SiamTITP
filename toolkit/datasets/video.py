...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        if load_img:
            ...
            shuffle = img_names[0].split('.')[-1]
            if shuffle == 'tiff':
                img = np.array(Image.open(self.img_names[0]))
            else:
    ...
    def __iter__(self):
        for i in range(len(self.img_names)):
            if self.imgs is not None:
                ...
                if self.img_names[0].split('.')[-1] == 'tiff':
                    yield np.array(Image.open(self.img_names[i])),self.gt_traj[i]
                else:
