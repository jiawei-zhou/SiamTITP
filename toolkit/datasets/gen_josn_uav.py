import numpy as np
import json
def gen_uav_json(root,sequence_info_list):
    js = {}
    for i in sequence_info_list:
        video_name = i['name']
        video_dir = i['path'].split('/')[-1]
        video_img_names, gt_rect = construct_sequence(root,i)
        video_attr = i['object_class']
        js[video_name] = {}
        ...
        gts = []
        for gt in gt_rect:
            gts.append(list(map(float,gt)))
        video_init_rect = gts[0]
        js[video_name]['gt_rect'] = gts
        js[video_name]['init_rect'] = video_init_rect
        js[video_name]['attr'] = [video_attr]
    json.dump(js, open('/home/zhoujiawei/train_dataset/UAV123/UAV123.json', 'w'), indent=4, sort_keys=True)

def construct_sequence(root,sequence_info):
        ...
        anno_path = '{}/{}'.format(root, sequence_info['anno_path'])

        ground_truth_rect = np.loadtxt(str(anno_path), delimiter=',', dtype=np.float64) 
        return frames, ground_truth_rect

root = '/home/zhoujiawei/train_dataset/UAV123'
sequence_info_list = [
...
if __name__=='__main__':
     gen_uav_json(root,sequence_info_list)