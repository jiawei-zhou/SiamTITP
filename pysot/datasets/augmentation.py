...
class Augmentation:
    ...
    def _mask_aug(self,image,bbox):
        bbox = list(map(int,bbox))
        for i in np.arange(len(bbox)):
            bbox[i] = bbox_clip(bbox[i],0,image.shape[0])
        bbox = Corner(bbox[0],bbox[1],bbox[2],bbox[3])
        range_x = np.arange(bbox.x1,bbox.x2)
        range_y = np.arange(bbox.y1,bbox.y2)
        w = bbox.x2 - bbox.x1
        h = bbox.y2 - bbox.y1
        mask = np.ones_like(image).astype(np.float32)
        channel_average = np.mean(image, axis=(0, 1))
        left_x = np.random.choice(range_x,size=1)
        left_y = np.random.choice(range_y,size=1)
        right_x = bbox_clip(left_x + int(w/4),0,bbox.x2)
        right_y = bbox_clip(left_y + int(h/4),0,bbox.y2)
        point_x = np.arange(left_x,right_x)
        for i in np.arange(left_y,right_y):
            point_y = np.ones_like(point_x) * (i)
            for j in range(mask.shape[2]):
                value = channel_average[j] / image[point_y,point_x,j] 
                mask[point_y,point_x,j] = value
        return image * mask
...
if __name__ == '__main__':
    aug = Augmentation(shift=0.0, scale=0.0, blur=0.0, flip=0.0, color=0.0,mask=1.0)
    img_path = '/home/zhoujiawei/tracking_model/SiamCAR-master/Template Feature.png'
