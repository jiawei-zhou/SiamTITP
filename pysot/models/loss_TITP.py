...
class  SiamTITPLossComputation(object):
    """
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, labels, reg_targets):
        ...
        label_cls, reg_targets = self.prepare_targets(locations, labels, reg_targets)   # 生成标签
        ...
        # centerness_target_neg_pos = torch.zeros_like(temp,dtype=torch.int64)
        # centerness_target_neg_pos[pos_inds] = 1 
        if pos_inds.numel() > 0:
            centerness_targets = self.compute_centerness_targets(reg_targets_flatten)   
            reg_loss = self.box_reg_loss_func(          # 由预测框与目标框的超出面积计算IOU损失
                box_regression_flatten,
                reg_targets_flatten,
                centerness_targets
            )

            centerness_loss = self.centerness_loss_func(    # 逼近中心概率
                centerness_flatten,
                centerness_targets
            )
        else:   # 若没有符合要求的真值则不计算回归损失和中心损失
...
def make_SiamTITP_loss_evaluator(cfg):
    loss_evaluator = SiamTITPLossComputation(cfg)
