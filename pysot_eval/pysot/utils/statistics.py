...
# @jit(nopython=True)
def distance_scale(rect1,rect2):
    r_square = (np.square(rect1[:,2]) + np.square(rect1[:,3])) / 4
    d_square = np.square(rect2[:,0]-rect1[:,0] + (rect2[:,2] - rect1[:,2]) / 2) \
            + np.square(rect2[:,1]-rect1[:,1] + (rect2[:,3] - rect1[:,3]) / 2)
    return r_square / (r_square + d_square)
...
# @jit(nopython=True)
def EIoU_overlap(gt_bb, result_bb, n_frame):
    thresholds_overlap = np.arange(0, 1.05, 0.05)
    EIoU_success = np.zeros(len(thresholds_overlap))
    iou = np.ones(len(gt_bb)) * (-1)
    Distance_scale = np.ones(len(gt_bb)) * (-1)
    mask = np.sum(gt_bb > 0, axis=1) == 4
    iou[mask] = overlap_ratio(gt_bb[mask], result_bb[mask])
    Distance_scale[mask] = distance_scale(gt_bb[mask], result_bb[mask])
    EIoU = iou[mask] * Distance_scale[mask]
    for i in range(len(thresholds_overlap)):
        EIoU_success[i] = np.sum(EIoU > thresholds_overlap[i]) / float(n_frame)
    return EIoU_success
