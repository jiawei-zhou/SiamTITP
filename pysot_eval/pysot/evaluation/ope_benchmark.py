...
from ..utils import overlap_ratio, success_overlap, success_error,EIoU_overlap
...
class OPEBenchmark:
    ...
    def eval_EIou(self,eval_trackers=None):
        """
        Args: 
            eval_trackers: list of tracker name or single tracker name
        Return:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    if self.dataset.name == 'LaSOT':
                        ...
                        gt_traj = gt_traj[video.absent == 1]
                        tracker_traj = tracker_traj[video.absent == 1]
                EIoU_success_ret_[video.name] = EIoU_overlap(gt_traj, tracker_traj, n_frame)
            EIoU_success_ret[tracker_name] = EIoU_success_ret_
        return EIoU_success_ret
    ...
    def show_result(self, success_ret, precision_ret=None,EIoU_success_ret=None,
        ...
        header = ("|{:^"+str(tracker_name_len)+"}|{:^9}|{:^16}|{:^11}|{:^9}|").format(
                "Tracker name", "Success", "Norm Precision", "Precision","EIoU")
        formatter = "|{:^"+str(tracker_name_len)+"}|{:^9.3f}|{:^16.3f}|{:^11.3f}|{:^9.3f}|"
        print('-'*len(header))
        print(header)
        print('-'*len(header))
        for tracker_name in tracker_names:
            ...
            if EIoU_success_ret is not None:
                EIoU = np.mean(list(EIoU_success_ret[tracker_name].values()))
            else:
                EIoU = 0
            ...
            if norm_precision_ret is not None:
                ...
                norm_precision = 0
            print(formatter.format(tracker_name, success, norm_precision, precision, EIoU))
        print('-'*len(header))
